...
# Update : 29 Nov 2019 (Since Nov 2018)
#-----------------------------------------------------------------------------
# see usage below
use strict;
use Tie::File;
...
extract specific barcodes:
    --r2      read2 fq file
    -b        barcode id
    -d        individual barcode id file (make sure the id sorted)
USAGE
  exit;
};
  ...
  foreach my $tag (keys %BCS){
    foreach my $cid (sort {$a<=>$b} keys %{$BCS{$tag}}){
      ...
      #while(@{$BCS{$tag}{$cid}}){
        ...
        (my $E = $bc) =~ s/_//g;
        die "$E is less than last bb($S). Make sure the IDs are ordered." if $E < $S;
        my $rd = 0;
        while(<R1>){
          my $iID = $_;
          my @b = &getIdBcode($iID); $S = $b[2];
          #my @R1 = ($iID  ,<R1>,<R1>,<R1>);
          #my @R2 = (<R2>,<R2>,<R2>,<R2>);
          if($S < $E){
            <R2>;<R2>;<R2>;<R2>;
          ...
          my $jID = <R2>;
          if($fmt eq "fq"){
            print Q1 &exchange($iID).<R1>.<R1>.<R1>;
            print Q2 &exchange($jID).<R2>.<R2>.<R2>;
          }
          if($fmt eq "fa"){
            print A1 $iID.<R1>; <R1>;<R1>;
            print A2 $jID.<R2>; <R2>;<R2>;
          }
          $STAT{'barcodes'} ++;
          last if $rd eq $rpb;
        }
...
sub getIdBcode {
  ...
  $id =~ /[@\/](\d+)_(\d+)_(\d+)\//;
  my @bcode= ($1,$2,$3);
  my @bc = ($bcode[0],"$bcode[0]_$bcode[1]_$bcode[2]","$bcode[0]$bcode[1]$bcode[2]");
  return(@bc);
}
