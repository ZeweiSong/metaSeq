...
echo info: output dir: "Assemble_Lv0/BC$bead/"

pfx="$sam/Assemble_Lv0/$bead/$tag"

echo
echo init and decmpress fastq files
#mkdir -p VSEARCH/$sam/$b1/$bead/
#pigz -dc $pfx/$bead.1.fq.gz > $pfx/$bead.1.fq
#pigz -dc $pfx/$bead.2.fq.gz > $pfx/$bead.2.fq

# echo
# echo Merge seq
# vsearch --threads $threads --fastq_mergepairs $pfx.1.fq \
# --reverse $pfx.2.fq \
...
--uc $pfx.merge.derep.preclustered.uc --minuniquesize 1 \
...
#
# metabbq map.pl $pfx.merge.derep.fasta $pfx.merge.derep.preclustered.uc \
...
vsearch --threads $threads --cluster_size  $pfx.merge.derep.preclustered.fasta \
